...
function createKnobDevice(count) { const components=[],configuration={},runtime={}; for(let i=0;i<count;i++){const row=Math.floor(i/4),col=i%4,id=`K${String(i+1).padStart(3,"0")}`; components.push({id,label:`Knob ${i+1}`,type:"knob",position:{x:58+col*110,y:35+row*45}}); configuration[id]={messageType:"cc",channel:1,number:20+i}; runtime[id]=0;} return createDevice("lab-16k","LAB-16K","16 knobs · 4 × 4 matrix",components,configuration,runtime); }
function createButtonDevice(count) { const components=[],configuration={},runtime={}; for(let i=0;i<count;i++){const row=Math.floor(i/4),col=i%4,id=`B${String(i+1).padStart(3,"0")}`; components.push({id,label:`Button ${i+1}`,type:"button",position:{x:58+col*110,y:35+row*45}}); configuration[id]={messageType:"note",channel:1,number:60+i,mode:"momentary"}; runtime[id]=0;} return createDevice("lab-16b","LAB-16B","16 buttons · 4 × 4 matrix",components,configuration,runtime); }
function createFaderDevice(count) { const components=[],configuration={},runtime={}; for(let i=0;i<count;i++){const id=`F${String(i+1).padStart(3,"0")}`; components.push({id,label:`Fader ${i+1}`,type:"fader",position:{x:55+i*110,y:80}}); configuration[id]={messageType:"cc",channel:1,number:21+i}; runtime[id]=0;} return createDevice("lab-4f","LAB-4F","4 faders",components,configuration,runtime); }
